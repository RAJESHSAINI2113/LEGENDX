...
async def _(event):
    ...
    await tbot.edit_message(
        ...
        buttons=[
            ...
            [Button.inline("❄️", data=f"torrentstop-{sender}|{chatid}|{msgid}")],
        ],
    )
...
async def paginate_news(event):
    ...
    await tbot.edit_message(
        ...
        buttons=[
            [
                Button.inline(
                    "👈", data=f"prevtorrent-{sender}|{search}|{num}|{chatid}|{msgid}"
                ),
                Button.inline("🖖", data=f"torrentstop-{sender}|{chatid}|{msgid}"),
                Button.inline(
                    "👉", data=f"nexttorrent-{sender}|{search}|{num}|{chatid}|{msgid}"
                ),
            ],
            [
                Button.inline(
